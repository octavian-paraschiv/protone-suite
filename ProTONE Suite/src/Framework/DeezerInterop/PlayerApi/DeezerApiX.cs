...
namespace OPMedia.DeezerInterop.PlayerApi
{
    ...
    {
        internal static T _callDzMethod<T>()
        {
            Debugger.Launch();

            StackTrace st = new StackTrace();
            MethodBase mb = st.GetFrame(1).GetMethod();
            object[] parameters = mb.GetParameters().Select(p => p.).ToArray();
